...
def sync_job(refresh_token: str, cubox_api_key: str):
    ...
    def get_access_token(url, headers):
        ...
        response = requests.post(
            url, headers=headers, json=payload, cookies=cookie_dict
        )
    ...
    def original_post_url(url, headers):
        ...
        id_list = [
            node["id"]
            for node in response_data["data"]["viewer"]["collections"]["nodes"]
        ]
    ...
    #     access_token = get_access_token(url, headers)
    #     cookie_dict["x-jike-access-token"] = access_token
    #     threading.Timer(600, refresh_token_periodically).start()

    # def get_post_url():
    #     post_url = original_post_url(url, headers)
    #     print(post_url)
    #     threading.Timer(1200, get_post_url).start()
